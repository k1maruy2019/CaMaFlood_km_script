...
do
    ...
    if [ $SCHH == "06" ]; then
       ...
       #    echo "./camaflood_post_vis_nc.py $EXP $CDATE $REGION $LON1 $LON2 $LAT1 $LAT2"
       #    ./camaflood_post_vis_nc.py $EXP $CDATE $REGION $LON1 $LON2 $LAT1 $LAT2
       echo "./camaflood_te_vis_nc_region.py $EXP $CDATE $REGION $LON1 $LON2 $LAT1 $LAT2"
       ./camaflood_te_vis_nc_region.py $EXP $CDATE $REGION $LON1 $LON2 $LAT1 $LAT2
       echo "./camaflood_ec_vis_nc_region.py $EXP $CDATE $REGION $LON1 $LON2 $LAT1 $LAT2"
       ./camaflood_ec_vis_nc_region.py $EXP $CDATE $REGION $LON1 $LON2 $LAT1 $LAT2
    fi
##
    SYEAR=${SYEAR}
    SMON=${SMON}
